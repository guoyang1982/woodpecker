                ...
                for(String ipAndPort : allIpAndPort){
                    ...
                    if(isAlive){
                        ...
                        if(StringUtils.isNotBlank(ipAndPort)){
                            ip.append("<button type=\"button\"  class=\"btn mini green\">").append(ipAndPort).append("</button>").append("</br>");
                            hasAlive = true;
                        }
                    }else {
                        // 当前节点非存活
                        ip.append("<button type=\"button\"  class=\"btn mini gray\" onclick=\" deleteUnusedAppIp(").append("'").append(appInfo.getAppName()).append("'").append(",").append("'").append(ipAndPort.replaceAll("\\.","-")).append("'").append(")\">").append(ipAndPort).append("</button>").append("</br>");
